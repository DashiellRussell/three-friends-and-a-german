...
export function ReportConfig({ setView, uuid }: ReportConfigProps) {
    ...
    const handleGen = async () => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3001"}/api/reports/generate?${params.toString()}`, {
                method: "GET",
                headers: { uuid },
            });
    ...
    const { rangeLabels } = useMemo<{ rangeLabels: Record<string, string> }>(() => {
...
export function ReportSuccessView({ setView }: { setView: (v: "entries" | "report-config" | "generating" | "report") => void }) {
    return (
        <div className="px-5 pt-8 pb-[100px]" style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            <p className="mb-6 pl-8 text-xs text-zinc-400">Report details are inside the generated PDF document.</p>
