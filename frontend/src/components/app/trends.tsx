...
export function Trends() {
  ...
  const [trendsData, setTrendsData] = useState<{ date: string, energy: number | null, sleep: number | null, symptoms?: string[] }[] | null>(null);
  ...
  const trendLabels = trendsData ? trendsData.map((c, i) => {
  ...
  const avgEnergy = validEnergy.length > 0 ? (validEnergy.reduce((a, b) => a + b, 0) / validEnergy.length).toFixed(1) : "0.0";
  const avgSleep = validSleep.length > 0 ? (validSleep.reduce((a, b) => a + b, 0) / validSleep.length).toFixed(1) : "0.0";
    ...
    <div className="px-5 pt-8 pb-[100px]">
      ...
      <div className="mb-3 rounded-2xl border border-zinc-100 bg-white p-5 transition-all hover:border-zinc-200 hover:shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="relative pl-4">
            ...
            <Sparkline data={energyD} labels={trendLabels} color="#18181b" fill height={52} highlight={energyD.length - 1} />
          </div>
        )}
      </div>
          ...
          <div className="relative pl-6">
            ...
            <div className="absolute left-0 top-0 bottom-5 right-0 flex flex-col justify-between text-[9px] text-zinc-300">
              <div className="relative w-full flex items-center">
                <span className="absolute left-0 w-4 font-medium text-right mt-1">12</span>
                <div className="ml-6 flex-1 border-t border-dashed border-zinc-200 mt-1" />
              </div>
              <div className="relative w-full flex items-center">
                <span className="absolute left-0 w-4 font-medium text-right mt-1">9</span>
                <div className="ml-6 flex-1 mt-1 opacity-0" />
              </div>
              <div className="relative w-full flex items-center">
                <span className="absolute left-0 w-4 font-medium text-right mt-1">6</span>
                <div className="ml-6 flex-1 border-t border-dashed border-zinc-200 mt-1" />
              </div>
              <div className="relative w-full flex items-center">
                <span className="absolute left-0 w-4 font-medium text-right mt-[3px]">3</span>
                <div className="ml-6 flex-1 mt-[3px] opacity-0" />
              </div>
              <div className="relative w-full flex items-center pb-2">
                <span className="absolute left-0 w-4 font-medium text-right mt-[3px]">0</span>
                <div className="ml-6 flex-1 border-t border-dashed border-zinc-200 mt-[3px]" />
              </div>
            </div>
            <div className="relative top-[4px] z-10">
              <Sparkline data={sleepD} labels={trendLabels} color="#818cf8" fill height={64} highlight={sleepD.length - 1} minStatic={0} maxStatic={12} />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="mb-3 rounded-2xl border border-zinc-100 bg-white p-5 transition-all hover:border-zinc-200 hover:shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="text-sm text-zinc-400">No symptoms reported this period</div>
        ) : (
          topSymptoms.map(([name, count]) => (
            <div key={name} className="mb-3">
              <div className="mb-1.5 flex justify-between">
                <span className="text-[13px] text-zinc-700 capitalize">{name.replace("_", " ")}</span>
                <span className="text-xs text-zinc-400">{count}x</span>
              </div>
              <div className="h-1.5 rounded-full bg-zinc-100">
                <div
                  className="h-full rounded-full bg-red-400 transition-all duration-500"
                  style={{ width: `${Math.min((count / (trendsData?.length || 1)) * 100, 100)}%` }}
                />
              </div>
            </div>
          ))
