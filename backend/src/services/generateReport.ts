...
export const generateReport = async (data: any) => {
    ...
    if (checkIns.length > 0) {
        const energySum = checkIns.reduce((sum: number, c: any) => sum + (c.energy || 0), 0);
        avgEnergy = energySum / checkIns.length;

        const sleepSum = checkIns.reduce((sum: number, c: any) => sum + (Number(c.sleep_hours) || 0), 0);
...
Keep it to a concise, professional 4-6 sentence executive summary. Do not use markdown formatting. Do not overuse em dashes.`,
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
        "HealthBrief · For personal reference only",
