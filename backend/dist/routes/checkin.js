...
router.post("/", async (req, res) => {
    ...
    const extracted = await (0, mistral_1.extractCheckinData)(transcript);
    // Step 2: embed the clean summary (not the noisy raw transcript)
    const embedding = await (0, mistral_1.embedText)(extracted.summary);
...
router.post("/summary", async (req, res) => {
    ...
    const context = await (0, mistral_1.generateConversationContext)(mapped);
    res.json({
...
router.post("/chat/start", async (req, res) => {
    ...
    const message = await (0, mistral_1.generateChatOpener)(systemPrompt);
    res.json({ message });
});
router.post("/chat/message", async (req, res) => {
    ...
    const message = await (0, mistral_1.generateChatReply)(systemPrompt, history);
    res.json({ message });
});
